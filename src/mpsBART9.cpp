...
void grow(Node* tree, modelParam &data, arma::vec &curr_res){
        ...
        cout << "Second round of likelihood calculations" << endl;
...
void Node::splineNodeLogLike(modelParam& data, arma::vec &curr_res){
        ...
        if(isRoot){
                ...
                arma::mat res_cov(data.x_train.n_rows,data.x_train.n_rows,arma::fill::zeros);
                arma::mat diag_aux = arma::eye(data.x_train.n_rows,data.x_train.n_rows);


                // Storing the new quantities for the node
                B = data.B_train;

                // Just to initialise those objects
                arma::mat b_t_ones_(data.p,data.d_var,arma::fill::zeros);
                arma::cube B_t_(data.p,data.x_train.n_rows,data.d_var,arma::fill::zeros);

                B_t = B_t_;
                b_t_ones = b_t_ones_;


                cout << "Slices transposed: " << B_t.n_slices << endl;
                for(int k = 0; k <data.d_var;k ++) {
                        B_t.slice(k) = data.B_train.slice(k).t();
                        arma::mat ones_vec(n_leaf,1,arma::fill::ones);
                        b_t_ones.col(k) = B_t.slice(k)*ones_vec;
                        B_test = data.B_test;
                        leaf_res = curr_res;
                }
        ...
        arma::cube B_(data.x_train.n_rows,data.p,data.d_var,arma::fill::zeros);
        arma::cube B_t_(data.p,data.x_train.n_rows,data.d_var,arma::fill::zeros);
        arma::cube B_test_(data.x_test.n_rows,data.p,data.d_var,arma::fill::zeros);
        arma::mat b_t_ones_(data.p,data.d_var,arma::fill::zeros);
        arma::vec leaf_res_ = arma::vec(n_leaf,arma::fill::zeros);

        // Some aux elements
        arma::mat ones_vec(data.x_train.n_rows,1,arma::fill::ones);
        arma::mat diag_aux(data.x_train.n_rows,data.x_train.n_rows,arma::fill::eye);
        arma::mat res_cov(data.x_train.n_rows,data.x_train.n_rows,arma::fill::zeros);
        ...
        for(int k = 0; k < data.d_var;k++){
                ...
                if(ancestors(k)>0){
                        ...
                        // Using the updated values to calculated the new quantities
                        res_cov = res_cov + (1/data.tau_b(k))*B_.slice(k)*data.P_inv*B_t_.slice(k); // Do not penalise by tree number of trees factor
                } else {
                        // Using previous values to calculate quantities
                        cout << "Here the current value of B doesnt work" << endl;
                        res_cov = res_cov + (1/data.tau_b(k))*B.slice(k)*data.P_inv*B_t.slice(k); // Using the update before the split
                }

                // Test elements
                for(int i = 0 ; i < n_leaf_test;i++){

                        if(ancestors(k)>0){ // Check to only use the basis from splits
        ...
        // Storing the new quantities for the node
        // ((UPDATE THOSE QUANTITIES ONLY FOR THE ANCESTORS))
        // === Only the ones where the's intercept ===
        for(int k = 0; k < data.d_var; k++){
                B.slice(k) = B_.slice(k);
                B_t.slice(k) = B_t_.slice(k);
                B_test.slice(k) = B_test_.slice(k);
        }
        b_t_ones = b_t_ones_;
        leaf_res = leaf_res_;
...
void getPredictions(Node* tree,
                    ...
                    arma::vec& current_prediction_train_one,
                    arma::vec& current_prediction_train_two,
                    arma::vec& current_prediction_train_three,
                    arma::vec& current_prediction_train_four,
        ...
        for(int i = 0; i<t_nodes.size();i++){
                ...
                arma::vec betas_b_sum_one(t_nodes[i]->n_leaf,arma::fill::zeros);
                arma::vec betas_b_sum_two(t_nodes[i]->n_leaf,arma::fill::zeros);
                arma::vec betas_b_sum_three(t_nodes[i]->n_leaf,arma::fill::zeros);
                arma::vec betas_b_sum_four(t_nodes[i]->n_leaf,arma::fill::zeros);
                ...
                for(int j = 0;j<data.d_var;j++){
                        ...
                        if(j==0){
                                betas_b_sum_one = t_nodes[i]->B.slice(j)*t_nodes[i]->betas.col(j);
                        }

                        if(j==1){
                                betas_b_sum_two = t_nodes[i]->B.slice(j)*t_nodes[i]->betas.col(j);
                        }

                        if(j==2){
                                betas_b_sum_three = t_nodes[i]->B.slice(j)*t_nodes[i]->betas.col(j);
                        }

                        if(j==3){
                                betas_b_sum_four = t_nodes[i]->B.slice(j)*t_nodes[i]->betas.col(j);
                        }
                ...
                for(int j = 0; j<data.x_train.n_rows; j++){
                        ...
                        current_prediction_train_one[t_nodes[i]->train_index[j]] = betas_b_sum_one[j];
                        current_prediction_train_two[t_nodes[i]->train_index[j]] = betas_b_sum_two[j];
                        current_prediction_train_three[t_nodes[i]->train_index[j]] = betas_b_sum_three[j];
                        current_prediction_train_four[t_nodes[i]->train_index[j]] = betas_b_sum_four[j];
...
Rcpp::List sbart(arma::mat x_train,
        ...
        for(int mcmc_iter = 0;mcmc_iter<data.n_mcmc;mcmc_iter++){
                ...
                for(int t = 0; t<data.n_tree;t++){
                        ...
                        getPredictions(all_forest.trees[t],
                                       data,
                                       y_hat,
                                       prediction_test,
                                       prediction_train_sum_one,
                                       prediction_train_sum_two,
                                       prediction_train_sum_three,
                                       prediction_train_sum_four,
                                       tree_mcmc_counter);
